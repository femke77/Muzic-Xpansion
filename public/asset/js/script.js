...
$("#ytSubmit").on("click", function (event) {
  ...
  if (input === "") {
    $("#modal-message").text(
      "Please enter an artist or band name in the search bar."
    );
    $("#oops-modal").modal("show");
  } else {
    input = input.split(" ").join("-");
    var queryURLPerformers =
      "https://api.seatgeek.com/2/performers?slug=" +
      input +
      "&client_id=" +
      seatGeekKey;
    ...
    $.ajax({
      ...
      if (response.performers.length === 0) {
        $("#modal-message").text(
          "Performer not found. Please check spelling and try again."
        );
        $("#oops-modal").modal("show");
      } else {
...
function getEvents() {
  ...
  $.ajax({
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      $eventListItem.append(
        "Location: " + results[i].venue.display_location + "<br/>"
      );
